    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-8 max-w-7xl mx-auto scroll-smooth">
      <div className="space-y-8 pb-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-[#1c1c1c] tracking-tight">Partner Ecosystem</h1>
            ...
            <table className="w-full text-left border-collapse cursor-pointer">
              ...
              <tbody className="divide-y divide-[#eaeaea]">
                {currentItems.length > 0 ? currentItems.map((partner, i) => (
                  <tr key={i} onClick={() => setSelectedPartner(partner)} className="hover:bg-[#f9f9f9] active:bg-gray-100 transition-colors group">
                    <td className="px-6 py-4 whitespace-nowrap">
                       <div className="flex items-center gap-3">
                         ...
                         <div>
                           <p className="text-sm font-extrabold text-[#1c1c1c]">{partner.name}</p>
                           <p className="text-[11px] text-gray-400 font-bold uppercase tracking-widest">{partner.id}</p>
                         </div>
                       </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
                       <span className="bg-[#f9f9f9] px-3 py-1.5 rounded-xl border border-[#eaeaea] text-[10px] font-black uppercase tracking-widest text-[#1c1c1c]">{partner.type}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap hidden lg:table-cell">
                       <p className="text-sm font-bold text-[#1c1c1c]">{partner.email}</p>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] uppercase font-black tracking-widest
                        ${partner.status === 'Verified' ? 'bg-gray-200 text-[#1c1c1c]' : 'bg-gray-50 text-black'}
                      `}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-black text-[#1c1c1c] text-right">{partner.listings} Services</td>
                    <td className="px-6 py-4 whitespace-nowrap text-right relative hidden lg:table-cell">
                       <button onClick={(e) => toggleDropdown(e, partner.id)} className="w-8 h-8 rounded-full flex items-center justify-center bg-transparent text-gray-400 hover:bg-gray-100 hover:text-[#1c1c1c] ml-auto focus:outline-none transition-colors">
                       ...
                       {openDropdown === partner.id && (
                         <div className="absolute right-6 top-10 mt-1 w-48 bg-white shadow-2xl border border-[#eaeaea] rounded-2xl overflow-hidden z-20">
                            <button onClick={(e) => { e.stopPropagation(); openEditModal(partner); }} className="w-full text-left px-5 py-3 text-xs font-bold text-[#1c1c1c] hover:bg-[#f9f9f9] border-b border-[#eaeaea] flex items-center gap-2"><Edit2 size={14}/> Edit Details</button>
                            {(partner.status === 'Pending Audit') ? (
                              <button onClick={(e) => { e.stopPropagation(); handleStatusChange(partner.id, 'Verified'); }} className="w-full text-left px-5 py-3 text-xs font-bold text-black hover:bg-[#f9f9f9] border-b border-[#eaeaea] flex items-center gap-2"><CheckCircle2 size={14}/> Validate Auth</button>
                            ) : (
                              <button onClick={(e) => { e.stopPropagation(); handleStatusChange(partner.id, 'Pending Audit'); }} className="w-full text-left px-5 py-3 text-xs font-bold text-black hover:bg-[#f9f9f9] border-b border-[#eaeaea] flex items-center gap-2"><FileWarning size={14}/> Revoke Validity</button>
                            )}
                            <button onClick={(e) => { e.stopPropagation(); handleDelete(partner.id); }} className="w-full text-left px-5 py-3 text-xs font-bold text-black hover:bg-gray-50 flex items-center gap-2"><Trash2 size={14}/> Offboard Partner</button>
                         </div>
                       )}
                    </td>
                  </tr>
                  ...
                  <tr>
                     <td colSpan="6">
                       <div className="text-center py-24 bg-white rounded-b-3xl">
                          ...
                          <h3 className="text-lg font-black text-[#1c1c1c] mb-1">No partners onboarded</h3>
      ...
      {selectedPartner && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center pt-20 px-4 pb-4">
           <div className="fixed inset-0 bg-[#1c1c1c]/60 backdrop-blur-sm transition-opacity" onClick={() => setSelectedPartner(null)} />
           <div className="relative w-full max-w-md bg-white rounded-[32px] shadow-2xl p-6 z-10 animate-slideUp sm:animate-scaleIn overflow-y-auto max-h-[85vh]">
             <div className="flex justify-between items-start mb-6">
                <div className="flex gap-4 items-center">
                   ...
                   <div>
                     <h3 className="text-2xl font-black text-[#1c1c1c]">{selectedPartner.name}</h3>
                     <p className="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest mt-1">{selectedPartner.id}</p>
                   </div>
                </div>
                <button onClick={() => setSelectedPartner(null)} className="w-10 h-10 bg-[#f9f9f9] rounded-full flex items-center justify-center text-[#1c1c1c] hover:bg-gray-200 transition-colors">
             ...
             <div className="space-y-5 mb-8 border-y border-[#eaeaea] py-6">
                <div className="flex justify-between bg-[#f9f9f9] p-4 rounded-2xl">
                   <div>
                     <p className="text-[10px] font-extrabold text-[#1c1c1c] uppercase tracking-widest mb-1">Category Type</p>
                     <p className="text-sm font-black text-[#1c1c1c]">{selectedPartner.type}</p>
                   </div>
                   <div className="text-right border-l border-[#eaeaea] pl-4">
                     <p className="text-[10px] font-extrabold text-[#1c1c1c] uppercase tracking-widest mb-1">Active Listings</p>
                     <p className="text-sm font-black text-[#1c1c1c]">{selectedPartner.listings}</p>
                   </div>
                </div>
                <div>
                   ...
                   <p className="text-[15px] font-bold text-[#1c1c1c] underline decoration-gray-300">{selectedPartner.email}</p>
                </div>
                <div>
                   ...
                   <p className="text-[15px] font-bold text-[#1c1c1c] underline decoration-gray-300">{selectedPartner.contact}</p>
                </div>
             </div>
             ...
             <div className="space-y-3">
               ...
               <div className="grid grid-cols-2 gap-3 mb-4">
                  <span className={`col-span-2 inline-flex items-center justify-center py-2.5 rounded-xl text-sm font-black uppercase tracking-widest border
                    ${selectedPartner.status === 'Verified' ? 'bg-gray-200 text-black border-black' : 'bg-gray-50 text-black border-gray-300'}
                  `}>
                      Current Status: {selectedPartner.status}
                  </span>
               </div>
               
               <button onClick={() => openEditModal(selectedPartner)} className="w-full py-3.5 bg-black text-white text-[#1c1c1c] font-black rounded-xl hover:bg-[#c9c9c9] transition-colors shadow-sm mb-2">Edit Setup</button>

               {(selectedPartner.status === 'Pending Audit') ? (
                  <button onClick={() => handleStatusChange(selectedPartner.id, 'Verified')} className="w-full py-3.5 bg-[#1c1c1c] text-white font-black rounded-xl hover:bg-black transition-colors shadow-sm">Verify Agency Status</button>
               ) : (
                  <button onClick={() => handleStatusChange(selectedPartner.id, 'Pending Audit')} className="w-full py-3 bg-white border border-[#eaeaea] text-[#1c1c1c] font-extrabold rounded-xl hover:bg-gray-50 transition-colors">Revoke Validity</button>
               )}
               <button onClick={() => handleDelete(selectedPartner.id)} className="w-full py-3 bg-gray-50 border border-gray-300 text-black font-extrabold rounded-xl hover:bg-gray-100 transition-colors mt-4">Offboard Agency</button>
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center font-sans px-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-[32px] shadow-2xl p-6 md:p-8 z-10 animate-scaleIn">
             <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-black text-[#1c1c1c]">{editingPartner ? "Edit Partner" : "Onboard Partner"}</h3>
             ...
             <div className="space-y-5">
                <div>
                   ...
                   <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-[#f9f9f9] text-sm font-bold text-[#1c1c1c] rounded-2xl px-5 py-4 border border-[#eaeaea] outline-none focus:border-[#1c1c1c] transition-colors" placeholder="e.g. Bali Transports" />
                </div>
                <div>
                   <label className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-2 block">Service Category</label>
                   <select value={formData.type} onChange={(e) => setFormData({...formData, type: e.target.value})} className="w-full bg-[#f9f9f9] text-sm font-bold text-[#1c1c1c] rounded-2xl px-5 py-4 border border-[#eaeaea] outline-none focus:border-[#1c1c1c] h-[52px]">
                ...
                <div>
                   ...
                   <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full bg-[#f9f9f9] text-sm font-bold text-[#1c1c1c] rounded-2xl px-5 py-4 border border-[#eaeaea] outline-none focus:border-[#1c1c1c] transition-colors" placeholder="admin@company.com" />
                </div>
                <div>
                   <label className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-2 block">Emergency Contact</label>
                   <input type="text" value={formData.contact} onChange={(e) => setFormData({...formData, contact: e.target.value})} className="w-full bg-[#f9f9f9] text-sm font-bold text-[#1c1c1c] rounded-2xl px-5 py-4 border border-[#eaeaea] outline-none focus:border-[#1c1c1c] transition-colors" placeholder="+62..." />
                </div>
             </div>
